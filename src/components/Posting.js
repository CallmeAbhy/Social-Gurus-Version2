...
const[knowloc,setKnowloc] = useState();
  ...
  const postjob = (event) => {
    ...
    let stipendamountmin = knowloc === 'fixed' ? stipendamountmax : parseInt(event.target.stipendamountmin.value);
    ...
    const database  = {jobprofile, workfromhome, openings, duration, responsibilities,skills,cities,stipendtype,stipendamountmax,stipendamountmin,  clientId, startDate, endDate};
...
<div class="form-check">
         <input class="form-check-input" type="radio" name='stipendtype'  value="fixed"  checked={ knowloc === 'fixed'}  onClick={ know_location }   />
         <label class="form-check-label" for="flexRadioDefault1">
           Fixed
         </label>
</div>
<div class="form-check">
  <input class="form-check-input" type="radio"  value="nominal" onClick={ know_location } checked={ knowloc === 'nominal'} />
  <label class="form-check-label" for="flexRadioDefault1">
    Nominal
  </label>
</div>
