...
<Navbar>
  ...
  <Container>
    ...
    <Button variant='dark' as={ Link } to="/Applied">My Applied Opportunities</Button>
    </Navbar.Collapse>
  </Container>
</Navbar>
...
<Row >
    ...
    Mode / Location :- <br />
    {
      curElem.workfromhome === true ? <Button variant='success' className="mt-3" disabled>Remote</Button> 
      : 
  ...
  <Row className='mt-3'>
    ...
    {
      curElem.stipendtype === "fixed" ? <p>Stipend :- ₹ {curElem.stipendamountmax} / Month</p>  : 
      <p>Stipend :- ₹ {curElem.stipendamountmax} / Month  (Performance Based)</p>
    }
    ...
    <Col xs={4} md={4} >
    <Button variant="outline-dark"  onClick={()=>opening(index)}>
          Apply
    </Button>
    {open  && (element === index) &&

      <Yourjob job_id = { jobid }  Company = {Company_name} Profile = {job_profile}  open = { open } setOpen ={setOpen} Responsible = { Responsible } Slots = {slots} />
  ...
  <Row>
  <marquee>The application is Vallid till <b className="blink">{date} </b> on <b className="blink">{result[0]}</b> </marquee>
